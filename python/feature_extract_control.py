...
# extracts features (see below) from each image from the control group
...
def main():
    ...
    for idx, fname in enumerate(os.listdir(data_dir)):
        ...
        img_dnois = cv2.fastNlMeansDenoisingColored(img, None, 35, 35, 7, 21) # (40, 40) works pretty well
        img_dnois = cv2.GaussianBlur(img_dnois, (11,11), 0) # (15, 15) works pretty well

        gray_img_dnois = cv2.cvtColor(img_dnois, cv2.COLOR_BGR2GRAY)
        ret, gray_img_dnois = cv2.threshold(gray_img_dnois, 5, 255, 1)
        ...
        # denoise
        img_neut_proc = cv2.fastNlMeansDenoising(img_neut, None, 60, 9, 21) # NOTE adjust this for num neutrophils detected (i.e. more noise filtered), default was (50,7,21)
        eh_ret, img_neut_proc = cv2.threshold(img_neut_proc, 225, 255, 0)
        img_neut_proc = cv2.dilate(img_neut_proc, np.ones((6,6), np.uint8), iterations=1)
    ...
    with open('extracted_features_control.json', 'w') as f:
