...
export class ChartB {
  ...
  drawCoordinate({max_AI,max_N}){
    ...
    this.xScale_I = d3.scaleBand()
    .domain(ITEMS)      
    .range([MARGINS.left, MARGINS.left+xScale_length])                      
    .padding([0.8])                          

    const xAxis_I = d3.axisBottom(this.xScale_I);
    ...
    //x-axis for alveoli
    this.xScale_A = d3.scaleBand()
    .domain(ITEMS)      
    .range([MARGINS.left+xScale_length, MARGINS.left+2*xScale_length])                      
    .padding([0.8])                          

    const xAxis_A = d3.axisBottom(this.xScale_A);
    ...
    this.xScale_N = d3.scaleBand()
    .domain(ITEMS)      
    .range([gap+MARGINS.left+2*xScale_length, gap+MARGINS.left+3*xScale_length])           
    .padding([0.8])                          

    const xAxis_N = d3.axisBottom(this.xScale_N);
  ...
  drawChart(){
    ...
    Promise.all([
      ...
      const [topmin_I,topmax_I] = d3.extent(topFeatures, d => d.interstitial_area);
      const [botmin_I,botmax_I] = d3.extent(botFeatures, d => d.interstitial_area);
      const topq1_I = d3.quantile(topFeatures, 0.25, d => d.interstitial_area);
      const botq1_I = d3.quantile(botFeatures, 0.25, d => d.interstitial_area);
      const topq2_I = d3.quantile(topFeatures, 0.50, d => d.interstitial_area);
      const botq2_I = d3.quantile(botFeatures, 0.50, d => d.interstitial_area);
      const topq3_I = d3.quantile(topFeatures, 0.75, d => d.interstitial_area);
      const botq3_I = d3.quantile(botFeatures, 0.75, d => d.interstitial_area);

      //get 5-statistics for alveoli area
      const [topmin_A,topmax_A] = d3.extent(topFeatures, d => d.alveoli_area);
      const [botmin_A,botmax_A] = d3.extent(botFeatures, d => d.alveoli_area);
      const topq1_A = d3.quantile(topFeatures, 0.25, d => d.alveoli_area);
      const botq1_A = d3.quantile(botFeatures, 0.25, d => d.alveoli_area);
      const topq2_A = d3.quantile(topFeatures, 0.50, d => d.alveoli_area);
      const botq2_A = d3.quantile(botFeatures, 0.50, d => d.alveoli_area);
      ...
      const [topmin_N,topmax_N] = d3.extent(topFeatures, d => d.neutrophil_area);
      const [botmin_N,botmax_N] = d3.extent(botFeatures, d => d.neutrophil_area);
      ...
      const topq2_N = d3.quantile(topFeatures, 0.50, d => d.neutrophil_area);
      const botq2_N = d3.quantile(botFeatures, 0.50, d => d.neutrophil_area);
      ...
      //top
     this.box_I.append("line")
     .attr("x1", xScale_I(ITEMS[0]))
     .attr("x2", xScale_I(ITEMS[0]))
     ...
     this.box_I.append("rect")
     .attr("x", xScale_I(ITEMS[0])-boxwidth/2)
     ...
     this.box_I.selectAll("bars")
     .data([topmin_I, topmax_I])
     .enter()
     .append("line")
     .attr("x1", xScale_I(ITEMS[0])-boxwidth/4)
     .attr("x2", xScale_I(ITEMS[0])+boxwidth/4)
     .attr("y1", function(d){ return(yScale(d))} )
     .attr("y2", function(d){ return(yScale(d))} )
     .attr("stroke", "#1f78b4")

     this.box_I.append("line")
     .attr("x1", xScale_I(ITEMS[0])-boxwidth/2)
     .attr("x2", xScale_I(ITEMS[0])+boxwidth/2)
     ...
     //bottom
     this.box_I.append("line")
     .attr("x1", xScale_I(ITEMS[1]))
     .attr("x2", xScale_I(ITEMS[1]))
     ...
     this.box_I.append("rect")
     .attr("x", xScale_I(ITEMS[1])-boxwidth/2)
     ...
     .enter()
     .append("line")
     .attr("x1", xScale_I(ITEMS[1])-boxwidth/4)
     .attr("x2", xScale_I(ITEMS[1])+boxwidth/4)
     .attr("y1", function(d){ return(yScale(d))} )
     .attr("y2", function(d){ return(yScale(d))} )
     .attr("stroke", "#1f78b4")

     this.box_I.append("line")
     .attr("x1", xScale_I(ITEMS[1])-boxwidth/2)
     .attr("x2", xScale_I(ITEMS[1])+boxwidth/2)
     ...
     //Box for alveoli area
      //top
      this.box_A.append("line")
      .attr("x1", xScale_A(ITEMS[0]))
      .attr("x2", xScale_A(ITEMS[0]))
      ...
      this.box_A.append("rect")
      .attr("x", xScale_A(ITEMS[0])-boxwidth/2)
      ...
      this.box_A.selectAll("bars")
      .data([topmin_A, topmax_A])
      .enter()
      .append("line")
      .attr("x1", xScale_A(ITEMS[0])-boxwidth/4)
      .attr("x2", xScale_A(ITEMS[0])+boxwidth/4)
      .attr("y1", function(d){ return(yScale(d))} )
      .attr("y2", function(d){ return(yScale(d))} )
      .attr("stroke", "#C1C1C1")
 
      this.box_A.append("line")
      .attr("x1", xScale_A(ITEMS[0])-boxwidth/2)
      .attr("x2", xScale_A(ITEMS[0])+boxwidth/2)
      ...
      //bottom
      this.box_A.append("line")
      .attr("x1", xScale_A(ITEMS[1]))
      .attr("x2", xScale_A(ITEMS[1]))
      ...
      this.box_A.append("rect")
      .attr("x", xScale_A(ITEMS[1])-boxwidth/2)
      ...
      .enter()
      .append("line")
      .attr("x1", xScale_A(ITEMS[1])-boxwidth/4)
      .attr("x2", xScale_A(ITEMS[1])+boxwidth/4)
      .attr("y1", function(d){ return(yScale(d))} )
      .attr("y2", function(d){ return(yScale(d))} )
      .attr("stroke", "#C1C1C1")
 
      this.box_A.append("line")
      .attr("x1", xScale_A(ITEMS[1])-boxwidth/2)
      .attr("x2", xScale_A(ITEMS[1])+boxwidth/2)
      ...
      //top
      this.box_N.append("line")
      .attr("x1", xScale_N(ITEMS[0]))
      .attr("x2", xScale_N(ITEMS[0]))
      ...
      this.box_N.append("rect")
      .attr("x", xScale_N(ITEMS[0])-boxwidth/2)
      ...
      this.box_N.selectAll("bars")
      .data([topmin_N, topmax_N])
      .enter()
      .append("line")
      .attr("x1", xScale_N(ITEMS[0])-boxwidth/4)
      .attr("x2", xScale_N(ITEMS[0])+boxwidth/4)
      .attr("y1", function(d){ return(yScale_N(d))} )
      .attr("y2", function(d){ return(yScale_N(d))} )
      .attr("stroke", "#9CCC9C")
 
      this.box_N.append("line")
      .attr("x1", xScale_N(ITEMS[0])-boxwidth/2)
      .attr("x2", xScale_N(ITEMS[0])+boxwidth/2)
      ...
      //bottom
      this.box_N.append("line")
      .attr("x1", xScale_N(ITEMS[1]))
      .attr("x2", xScale_N(ITEMS[1]))
      ...
      this.box_N.append("rect")
      .attr("x", xScale_N(ITEMS[1])-boxwidth/2)
      ...
      .enter()
      .append("line")
      .attr("x1", xScale_N(ITEMS[1])-boxwidth/4)
      .attr("x2", xScale_N(ITEMS[1])+boxwidth/4)
      .attr("y1", function(d){ return(yScale_N(d))} )
      .attr("y2", function(d){ return(yScale_N(d))} )
      .attr("stroke", "#9CCC9C")
 
      this.box_N.append("line")
      .attr("x1", xScale_N(ITEMS[1])-boxwidth/2)
      .attr("x2", xScale_N(ITEMS[1])+boxwidth/2)
